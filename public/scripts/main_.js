...
//つまり最初にこれが動く
//画面表示の設定をする関数を動かす
//次の画面で使用する数値をローカルストレージに設定する（なぜかたまにうまくいかない！！！！）
...
//ローカルストレージに再度時間を設定する
//各種入力項目のチェック処理
//マッチング中かそうじゃないかでボタンの表示とかを変更
//FireBaseの機能でRoomsというコレクションを作成、既にWaitingのRoomsがあるならその部屋に自分のIDを設定してマッチング扱いにする
...
//「onSnapshot(roomsRef, async (snapshot) => {」これでRoomsが更新を監視する
